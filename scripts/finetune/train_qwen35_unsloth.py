...
- Uses FastModel + UnslothVisionDataCollator for the unified Qwen3.5 model
- Trains on multimodal "messages" format (text prompt + multiple images)
- Uses action-only JSON by default; rationale remains an optional target mode

Default split:
- Train and evaluation are separated by recording block, never channel number.
- The exact CH3/CH31 block is reserved for final testing and is not a default
...
def _assistant_target(row: Dict[str, Any], target_mode: str = "action_json") -> str:
...
def _filter_rows(
    rows: List[Dict[str, Any]],
    recording_blocks: Sequence[str],
    expert_only: bool,
) -> List[Dict[str, Any]]:
    keep = []
    block_set = set(recording_blocks)
    for row in rows:
        if str(row.get("recording_block", "")) not in block_set:
            continue
...
def main() -> None:
    ...
    parser.add_argument("--max-seq-length", type=int, default=0, help="0 disables truncation for VLM image tokens")
    ...
    parser.add_argument(
        "--target-mode",
        default="action_json",
        choices=["original", "action_json", "reasoned_json"],
        help="action_json is the recommended first SFT target; reasoning is optional",
    )
    ...
    train_rows = _filter_rows(all_rows, recording_blocks=train_blocks, expert_only=expert_only)
    eval_rows = _filter_rows(all_rows, recording_blocks=eval_blocks, expert_only=expert_only)
    if not train_rows:
