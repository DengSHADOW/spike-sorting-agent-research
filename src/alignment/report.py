...
def aggregate_setting_reports(
    ...
    """
    Aggregate alignment_report.json files for selected channels in a setting.

    Args:
        setting_id: Simulation setting identifier.
        output_dir: Root simulation output directory.
        channel_ids: Optional exact channel set for this evaluation run. When
            omitted, aggregate all available reports for backwards compatibility.
    ...
    reports: list[dict] = []
    requested_channel_ids = sorted(channel_ids) if channel_ids is not None else None
    channel_dirs = (
        [setting_dir / channel_id for channel_id in requested_channel_ids]
        if requested_channel_ids is not None
        else sorted(d for d in setting_dir.iterdir() if d.is_dir() and d.name.startswith("ch_"))
    )
    ...
    for ch_dir in channel_dirs:
        ...
        elif requested_channel_ids is not None:
            missing_channel_ids.append(ch_dir.name)
