...
class TrajectoryRunner:
    ...
    def __init__(
        ...
        rag_overwrite_memory: bool = False,
    ) -> None:
        self.cfg = cfg
        ...
        self.rag_overwrite_memory = rag_overwrite_memory
        self.rag_memory = (
            ContinualRAGMemory(
        ...
        # A single runner is shared across all requested channels by the Stage 3
        # script. Clear here (rather than per channel) so a fresh run can still
        # retrieve examples accumulated earlier in the same run.
        if self.rag_memory is not None and self.rag_overwrite_memory:
            self.rag_memory.clear(persist=rag_memory_path is not None)
    ...
    def run(
        ...
        for cid in phase1_clusters:
            ...
            # A non-KEEP oracle action must advance the assignments.  Do not
            # turn an unexecutable split into a canonical trajectory example.
            if oracle.action_type != "KEEP":
                assigns_before = manager.assigns.copy()
                try:
                    apply_method = apply_oracle_action(manager, oracle)
                except Exception as exc:
                    termination_reason = "action_failed"
                    blocked_action = {
                        "step": current_step,
                        "action_type": oracle.action_type,
                        "cluster_id": cid,
                    ...
                    print(f"  [warn] {channel_id}: could not apply {oracle.action_type} to cluster {cid}")
                    break
                if np.array_equal(assigns_before, manager.assigns):
                    ...
                    print(f"  [warn] {channel_id}: {oracle.action_type} made no progress for cluster {cid}")
                    break
            ...
            step = TrajectoryStep(
                ...
                rag_enabled=bool(retrieved_examples),
                rag_hits=len(retrieved_examples),
                retrieved_examples=retrieved_examples,
            )
            steps.append(step)
        ...
        active = [] if blocked_action is not None else manager.get_active_clusters()
        ...
        summary["completed"] = blocked_action is None
