...
def evaluate_on_channels(
    ...
    for channel_id in channel_ids:
        ...
        for cid in list(manager.get_active_clusters()):
            ...
            # Match Stage 2/3 semantics: an oracle action that leaves the
            # assignment unchanged is not a valid evaluation target.
            if oracle.action_type != "KEEP":
                assigns_before = manager.assigns.copy()
                try:
                    apply_oracle_action(manager, oracle)
                except Exception as exc:
                    print(f"  [warn] {channel_id}: could not apply {oracle.action_type} to cluster {cid}: {exc}")
                    break
