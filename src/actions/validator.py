...
def validate_trajectory(steps: list[dict]) -> ValidationResult:
    ...
    for i, step in enumerate(steps):
        ...
        n_before = step.get("n_active_before")
        n_after = step.get("n_active_after")
        if isinstance(n_before, int) and isinstance(n_after, int):
            if action_type == "SPLIT" and n_after <= n_before:
                errors.append(
                    f"Step {i}: SPLIT made no progress "
                    f"(active clusters {n_before} -> {n_after})."
                )
            elif action_type in {"MERGE", "DISCARD"} and n_after >= n_before:
                errors.append(
                    f"Step {i}: {action_type} made no progress "
                    f"(active clusters {n_before} -> {n_after})."
                )
