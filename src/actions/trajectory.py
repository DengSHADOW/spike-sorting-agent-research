...
def build_gt_trajectory(
    ...
    blocked_action: Optional[dict] = None

    for step_idx in range(max_steps):
        active = manager.get_active_clusters()
        if not active:
            completed = True
            termination_reason = "no_active_clusters"
            break
        ...
        if action.action_type == "KEEP":
            ...
            steps.append(record)
            completed = True
            termination_reason = "all_clusters_keep"
            break

        assigns_before = manager.assigns.copy()
        try:
            apply_method = apply_oracle_action(manager, action)
            ...
            termination_reason = "action_failed"
            print(
                f"  [warn] {raw_dir.parent.name}: could not apply "
                f"{action.action_type} to cluster {action.cluster_id}: {exc}"
            )
            break

        active_after = manager.get_active_clusters()
        state_changed = not np.array_equal(assigns_before, manager.assigns)
        if not state_changed:
            info = manager.get_cluster_info(action.cluster_id)
            blocked_action = {
                **record,
                "n_active_after": len(active_after),
                "applied": False,
                "apply_method": apply_method,
                "termination_reason": "split_no_progress",
                "n_overclusters": int(info["n_overclusters"]) if info else 0,
            }
            termination_reason = "split_no_progress"
            print(
                f"  [warn] {raw_dir.parent.name}: SPLIT made no progress for "
                f"cluster {action.cluster_id}; stopping trajectory."
            )
            break

        record["n_active_after"] = len(active_after)
        record["applied"] = True
        record["apply_method"] = apply_method
        steps.append(record)
    ...
    if blocked_action is not None:
        summary["blocked_action"] = blocked_action
...
def apply_oracle_action(manager: ClusterManager, action: OracleAction) -> str:
    """Apply one oracle action and return the implementation path used."""
    if action.action_type == "SPLIT":
        try:
            manager.split_last_merge(action.cluster_id)
            return "split_last_merge"
        except Exception:
            manager.split_by_current_overclusters(action.cluster_id)
            return "split_by_current_overclusters"
    elif action.action_type == "DISCARD":
        manager.discard_cluster(action.cluster_id)
        return "discard_cluster"
    ...
    raise ValueError(f"Unsupported oracle action: {action.action_type}")


# Backward-compatible private alias for callers that used the earlier helper.
_apply_action = apply_oracle_action
