...
Expected structure:
  spikes/
    ...
    curation/action(s): optional human action log
    curation/action_reasoning: optional human reasoning text
"""
...
def _decode_hdf5_string_refs(group: h5py.File, dataset_path: str) -> list[str]:
    """Decode a MATLAB v7.3 cell array whose entries reference char arrays."""
    if dataset_path not in group:
        return []

    values: list[str] = []
    for ref in np.asarray(group[dataset_path]).reshape(-1):
        if not ref:
            values.append("")
            continue
        chars = np.asarray(group[ref]).reshape(-1)
        values.append("".join(chr(int(value)) for value in chars if int(value)))
    return values


def _normalize_matlab_strings(value: Any) -> list[str]:
    """Normalize MATLAB v5 char/cell values to a plain list of strings."""
    if value is None:
        return []
    array = np.asarray(value, dtype=object)
    return [str(item).strip() for item in array.reshape(-1)]
...
def load_matlab_spikes(mat_path: str) -> Dict[str, Any]:
    ...
    Returns:
        ...
        - curation_actions: normalized human action strings (possibly empty)
        - curation_action_reasoning: normalized reasoning strings (possibly empty)
    """
    ...
    try:
        with h5py.File(mat_path, 'r') as f:
            ...
            action_path = (
                'spikes/curation/actions'
                if 'spikes/curation/actions' in f
                else 'spikes/curation/action'
            )
            data['curation_actions'] = _decode_hdf5_string_refs(f, action_path)
            data['curation_action_reasoning'] = _decode_hdf5_string_refs(
                f, 'spikes/curation/action_reasoning'
            )
    except Exception:
        ...
        if curation is not None:
            ...
            data['curation_actions'] = _normalize_matlab_strings(
                get_attr(curation, 'actions', get_attr(curation, 'action'))
            )
            data['curation_action_reasoning'] = _normalize_matlab_strings(
                get_attr(curation, 'action_reasoning')
            )
