...
def build_phase1_prompt(
    ...
    n_spikes: int,
    n_overclusters: int,
    isi_violation_rate: float | None = None,
    amplitude_cv: float | None = None,
) -> str:
    """Build Phase 1 VLM prompt with cluster context."""
    metric_lines = []
    if isi_violation_rate is not None:
        metric_lines.append(f"- ISI violation rate: {isi_violation_rate:.2%}")
    if amplitude_cv is not None:
        metric_lines.append(f"- Amplitude CV: {amplitude_cv:.3f}")
    metric_text = "\n".join(metric_lines)
    if metric_text:
        metric_text = "\n" + metric_text
    return f"""
...
- Composed of {n_overclusters} overclusters (hierarchical subcomponents){metric_text}

Use all four attached views in this fixed order: waveform overlay, ISI
histogram, amplitude distribution, and aggregation tree. Numeric metrics are
supporting evidence; do not infer a decision from cluster size alone.
...
- "NOT_MERGE" means: "These are different units; try other large clusters"
- If all candidates are "NOT_MERGE", preserve the small cluster as a distinct unit
