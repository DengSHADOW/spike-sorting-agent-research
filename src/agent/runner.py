...
try:
    ...
    VLM_AVAILABLE = False
    print("[Warning] VLM API support unavailable; real calls will fail unless --use-mock is explicit")
...
def create_waveform_overlay_image(
    ...
    if n_spikes > max_waveforms:
        # Deterministic sampling keeps the visual observation identical across
        # base/SFT/RAG comparisons and across resumed runs.
        indices = np.linspace(0, n_spikes - 1, max_waveforms, dtype=int)
    ...
    # A LineCollection avoids hundreds of Python-level artist calls while
    # preserving every selected waveform in the overlay.
    x_values = np.broadcast_to(time_ms, plot_waveforms.shape)
    segments = np.stack((x_values, plot_waveforms), axis=-1)
    ax.add_collection(
        LineCollection(segments, colors='steelblue', alpha=0.15, linewidths=0.6)
    )
    ax.set_xlim(float(time_ms[0]), float(time_ms[-1]))
    y_min = float(np.min(plot_waveforms))
    y_max = float(np.max(plot_waveforms))
    margin = max((y_max - y_min) * 0.05, 1e-9)
...
def create_aggregation_tree_image(
    ...
    plot_scale = max(max_y, 1e-3)
    y_top = plot_scale * 1.15
    ...
    for root_idx, root in enumerate(tree_roots):
        ...
        # Label
        stagger = (plot_scale * 0.05) * (root_idx % 3)
    ...
    ax.set_ylim(-plot_scale * 0.05, y_top * 1.15)
...
def create_waveform_comparison_image(
    ...
    """
    # Reuse overlay image utility with the same deterministic 500-waveform cap
    # used by the runtime observation contract.
...
def call_vlm_api(
    ...
    """
    # Mock mode must be explicit. Silently substituting mock decisions after a
    # provider/import failure would make a real scientific run invalid.
    if not VLM_AVAILABLE and not use_mock:
        raise RuntimeError("VLM API support is unavailable; refusing to substitute mock output")
    if use_mock:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                raise RuntimeError(
                    f"VLM provider failed after {max_retries} attempts; no mock fallback was used"
                ) from e
    ...
    for parse_attempt in range(max_parse_attempts):
        ...
        try:
            ...
            if action not in ["KEEP", "DISCARD"]:
                raise ValueError(f"Invalid neuronal action: {action!r}")
            return {
                "action": action,
                "rationale": decision.get("rationale", decision.get("reason", "")),
            ...
            if parse_attempt < max_parse_attempts - 1:
                ...
                print(f"[VLM] All {max_parse_attempts} parse attempts failed; returning ABSTAIN")
                return {
                    "action": "ABSTAIN",
...
def vlm_phase1_cluster_decision(
    ...
    wf_overlay = create_waveform_overlay_image(waveforms, cluster_id, sampling_rate)
    isi_hist = create_isi_histogram_image(spike_times, cluster_id)
    amplitude_viz = create_amplitude_distribution_image(waveforms, cluster_id)
    tree_viz = create_aggregation_tree_image(hierarchy_tree, overcluster_composition, cluster_id)

    if len(spike_times) < 2:
        isi_violation_rate = 0.0
    else:
        isi_violation_rate = float(np.mean(np.diff(np.sort(spike_times)) < 0.002))
    amplitudes = np.ptp(waveforms, axis=1) if len(waveforms) else np.array([])
    mean_amplitude = float(np.mean(amplitudes)) if len(amplitudes) else 0.0
    amplitude_cv = (
        float(np.std(amplitudes) / mean_amplitude) if mean_amplitude > 1e-12 else 0.0
    )
    ...
    save_meta = _save_vlm_inputs(
        ...
        images=[wf_overlay, isi_hist, amplitude_viz, tree_viz],
        prompt=prompt,
        image_names=["waveform", "isi", "amplitude", "tree"],
        extra_meta={
    ...
    for parse_attempt in range(max_parse_attempts):
        ...
        try:
            ...
            if action not in {"KEEP", "DISCARD", "SPLIT"}:
                raise ValueError(f"Invalid Phase 1 action: {action!r}")
            return {
                "action": action,
                "rationale": decision.get("rationale", decision.get("reason", "")),
                "raw_response": raw_response,
...
def vlm_phase2_merge_decision(
    ...
    Returns:
        Dict with:
            ...
            - image_paths: Saved image paths (if output_dir is provided)
    """
    # Use the same deterministic 500-waveform cap as training export.
    small_wf_img = create_waveform_overlay_image(
        waveforms=small_waveforms,
        cluster_id=small_cluster_id,
        sampling_rate=sampling_rate,
    ...
    for parse_attempt in range(max_parse_attempts):
        ...
        try:
            ...
            if action not in {"MERGE", "NOT_MERGE", "DISCARD"}:
                raise ValueError(f"Invalid Phase 2 action: {action!r}")
            return {
                "action": action,
                "rationale": decision.get("rationale", decision.get("reason", "")),
                "raw_response": raw_response,
