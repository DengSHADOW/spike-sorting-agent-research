"""
VLM-driven curation pipeline with optional, explicit size filters.
...
class PureVLMCurationPipeline:
    """
    VLM-driven curation pipeline.

    Rule-based size filters are optional and are reported separately from VLM
    decisions. An invalid/unparseable decision is recorded as ABSTAIN and never
    mutates cluster assignments.
    """
    ...
    def run_phase0_automatic_filtering(self) -> List[int]:
        ...
        """
        if self.auto_discard_threshold <= 0:
            kept = self.manager.get_active_clusters()
            print(f"\n[Phase 0] Disabled; preserving {len(kept)} clusters")
            return kept

        print("\n" + "="*70)
        print(f"PHASE 0: AUTOMATIC SIZE FILTERING (<{self.auto_discard_threshold} spikes)")
        print("="*70)
        ...
        if action == "DISCARD":
            ...
            try:
                ...
                self.log_action(
                    action="ABSTAIN",
                    cluster_ids=[cluster_id],
                    reason=f"Requested split could not be applied: {e}",
                    phase=f"Phase1_Iter{iteration}",
                )
                return [cluster_id]
            ...
            print(f"  ⚠ VLM: ABSTAIN on cluster {cluster_id}; state preserved")
            self.log_action(
                action="ABSTAIN",
                cluster_ids=[cluster_id],
                reason=f"VLM: {rationale}",
            ...
            print(f"  ⚠ Unknown VLM action '{action}'; preserving cluster {cluster_id}")
            self.log_action(
                action="ABSTAIN",
                cluster_ids=[cluster_id],
                reason=f"Unknown VLM action: {action!r}",
                phase=f"Phase1_Iter{iteration}",
            )
            return [cluster_id]
    ...
    def run_phase2_vlm_merge_decisions(
        ...
        4. If no valid merge target is found, preserve the unresolved cluster
        ...
        if not large_ids:
            print("⚠ No large clusters; preserving unresolved small clusters")
            for cid in small_ids:
                self.log_action(
                    action="ABSTAIN",
                    cluster_ids=[cid],
                    reason="No large clusters available as merge targets; state preserved",
                    phase="Phase2",
                )
            return self.manager.get_active_clusters()
        ...
        for large_cid in large_ids:
            ...
            if decision['action'] == "DISCARD":
                ...
                print(
                    f"  ⚠ VLM returned {decision['action']} for large cluster {large_cid}; "
                    "preserving but not using as target"
                )
                self.log_action(
                    action="ABSTAIN",
                    cluster_ids=[large_cid],
                    reason=(
                        f"VLM pre-validation returned {decision['action']}; "
                        f"state preserved: {decision['rationale']}"
                    ),
                    phase="Phase2_PreValidation",
                )
            else:
                print(f"  ✓ Large cluster {large_cid} validated with KEEP")
        ...
        if not valid_large_ids:
            print("⚠ No validated large clusters; preserving unresolved active clusters")
            for cid in small_ids:
                self.log_action(
                    action="ABSTAIN",
                    cluster_ids=[cid],
                    reason="No validated large cluster available; state preserved",
                    phase="Phase2",
                )
            return self.manager.get_active_clusters()
        ...
        for small_cid in small_ids:
            ...
            for large_cid in valid_large_ids:
                ...
                if action == "MERGE":
                    ...
                    resolved = True  # Stop trying other large clusters
                    break

                elif action == "ABSTAIN":
                    comparison_abstained = True
                    print(f"  ⚠ VLM: ABSTAIN for {small_cid} vs {large_cid}; trying other targets")
                    self.log_action(
                        action="ABSTAIN",
                        cluster_ids=[small_cid, large_cid],
                        reason=f"VLM: {decision['rationale']}",
            ...
            if not resolved:
                reason = (
                    "At least one comparison abstained; unresolved cluster preserved"
                    if comparison_abstained
                    else "All evaluated pairs returned NOT_MERGE; distinct cluster preserved"
                )
                print(f"  → {reason}: {small_cid}")
                self.log_action(
                    action="ABSTAIN" if comparison_abstained else "KEEP",
                    cluster_ids=[small_cid],
                    reason=reason,
                    phase="Phase2",
                )
    ...
    def run_phase3_final_filter(self, cluster_ids: List[int]) -> List[int]:
        ...
        """
        if self.final_minimum_threshold <= 0:
            print(f"\n[Phase 3] Disabled; preserving {len(cluster_ids)} clusters")
