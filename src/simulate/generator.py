...
def generate_recording(
    ...
    # MEArec otherwise defaults to all visible CPUs.  Each worker pre-generates
    # a large position array, which can exhaust a memory-limited WSL VM.  Keep
    # the safe default serial and allow explicit opt-in to more workers.
    template_n_jobs = max(1, int(os.environ.get("MEAREC_TEMPLATE_N_JOBS", "1")))
    ...
    if cfg.drift_enabled:
        ...
        drift_dict = {
            ...
            # MEArec 1.11 indexes the complete drift schema when callers pass
            # a partial recordings dictionary, including for rigid drift.
            "preferred_dir": [0, 0, 1],
            "angle_tol": 15,
            "n_drifting": None,
            "drift_fs": 100,
