...
def overcluster_recording(
    ...
    if len(unit_ids) == 0:
        if cfg.forced_merge_pairs or cfg.noise_clusters:
            raise RuntimeError(
                f"{cfg.setting_id}/{channel_id}: sorter returned no units, so requested "
                "controlled curation perturbations cannot be applied."
            )
    ...
    meta["curation_perturbations"] = perturbation_meta
    with open(raw_dir / "metadata.json", "w") as f:
        json.dump(meta, f, indent=2)
...
def _apply_curation_perturbations(
    cfg: SettingConfig,
    channel_idx: int,
    ...
    """Inject deterministic, GT-labelled initial errors for coverage experiments.

    A forced merge pair is made by splitting one otherwise clean hierarchy
    cluster into two active labels with the same dominant GT unit.  A noise
    cluster contains synthetic non-neuronal waveforms and GT label 0.  Both
    transformations are disabled by default and operate only on simulated raw
    arrays after sorting.
    """
    n_merge_pairs = max(0, int(cfg.forced_merge_pairs))
    n_noise_clusters = max(0, int(cfg.noise_clusters))
    noise_spikes = max(1, int(cfg.noise_spikes_per_cluster))
    meta = {
        "forced_merge_pairs_requested": n_merge_pairs,
        "forced_merge_pairs_applied": 0,
        "noise_clusters_requested": n_noise_clusters,
        "noise_clusters_applied": 0,
        "noise_spikes_per_cluster": noise_spikes if n_noise_clusters else 0,
    }
    if n_merge_pairs == 0 and n_noise_clusters == 0:
        return spike_times, waveforms, overcluster_assigns, hierarchy_assigns, gt_assigns, meta

    rng = np.random.default_rng(int(cfg.seed) * 1009 + int(channel_idx))
    hierarchy_assigns = hierarchy_assigns.copy()
    max_label = int(max(
        np.max(overcluster_assigns, initial=0),
        np.max(hierarchy_assigns, initial=0),
    ))

    # Split distinct clean hierarchy clusters. Each pair remains a valid MERGE
    # target because both children retain the same GT-dominant composition.
    candidates: list[tuple[int, np.ndarray]] = []
    for cluster_id in sorted(int(cid) for cid in np.unique(hierarchy_assigns) if cid != 0):
        indices = np.flatnonzero(hierarchy_assigns == cluster_id)
        labels = gt_assigns[indices]
        labels = labels[labels != 0]
        if len(indices) < 2 or labels.size != len(indices):
            continue
        _, counts = np.unique(labels, return_counts=True)
        if int(counts.max()) / len(indices) >= 0.95:
            candidates.append((cluster_id, indices))

    if len(candidates) < n_merge_pairs:
        raise RuntimeError(
            f"{cfg.setting_id}/ch_{channel_idx:03d}: requested {n_merge_pairs} forced merge pairs, "
            f"but only {len(candidates)} clean hierarchy clusters are available."
        )

    for _, indices in candidates[:n_merge_pairs]:
        selected = rng.permutation(indices)[: len(indices) // 2]
        max_label += 1
        hierarchy_assigns[selected] = max_label
        meta["forced_merge_pairs_applied"] += 1

    if n_noise_clusters:
        if waveforms.ndim != 2 or waveforms.shape[1] == 0:
            raise RuntimeError(f"{cfg.setting_id}/ch_{channel_idx:03d}: cannot add noise cluster without waveforms.")
        waveform_scale = float(np.std(waveforms))
        waveform_scale = waveform_scale if waveform_scale > 1e-8 else 1.0
        start = float(spike_times.min()) if spike_times.size else 0.0
        stop = float(spike_times.max()) if spike_times.size else max(float(cfg.duration), 1.0)

        added_times: list[np.ndarray] = []
        added_waveforms: list[np.ndarray] = []
        added_labels: list[np.ndarray] = []
        for _ in range(n_noise_clusters):
            max_label += 1
            added_times.append(rng.uniform(start, stop, size=noise_spikes))
            added_waveforms.append(
                rng.normal(0.0, waveform_scale, size=(noise_spikes, waveforms.shape[1])).astype(waveforms.dtype)
            )
            added_labels.append(np.full(noise_spikes, max_label, dtype=hierarchy_assigns.dtype))
            meta["noise_clusters_applied"] += 1

        spike_times = np.concatenate([spike_times, *added_times])
        waveforms = np.concatenate([waveforms, *added_waveforms])
        noise_assigns = np.concatenate(added_labels)
        overcluster_assigns = np.concatenate([overcluster_assigns, noise_assigns])
        hierarchy_assigns = np.concatenate([hierarchy_assigns, noise_assigns])
        gt_assigns = np.concatenate([gt_assigns, np.zeros(len(noise_assigns), dtype=gt_assigns.dtype)])

        order = np.argsort(spike_times, kind="stable")
        spike_times = spike_times[order]
        waveforms = waveforms[order]
        overcluster_assigns = overcluster_assigns[order]
        hierarchy_assigns = hierarchy_assigns[order]
        gt_assigns = gt_assigns[order]
